      ...
      <div className="flex items-center justify-center h-full bg-slate-50">
        <div className="text-center p-6 max-w-xs">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Make sure "Maps JavaScript API" is enabled in your Google Cloud Console.
          </p>
        </div>
      </div>
    );
      ...
      {selectedPlace && (
        ...
        >
          <div className="max-w-[200px] font-sans">
            {selectedPlace.photo && (
              <img src={selectedPlace.photo} alt={selectedPlace.name} className="w-full h-24 object-cover rounded mb-2" />
            )}
...
function AddToTripModal({ place, trips, onClose, defaultTripId }) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-4">
          {success ? (
            ...
            <>
              {/* Trip selector — hidden when defaultTripId locks it */}
              {!defaultTripId && (
                <div>
              ...
              {defaultTripId && trip && (
                <p className="text-xs text-slate-500">
                  Adding to <span className="font-semibold text-slate-700">{trip.name}</span>
                </p>
              )}
...
// ── ExplorePanel ──────────────────────────────────────────────────────────────
// Reusable component used both in ExplorePage (full screen) and inside trip tabs (embedded).
// Props:
//   defaultLocation — pre-fills the location field on first render
//   currentTripId   — pre-selects this trip in the AddToTripModal
//   embedded        — if true, uses a fixed height instead of full-viewport height

export default function ExplorePanel({ defaultLocation = '', currentTripId = null, embedded = false }) {
  ...
  // Update location field when defaultLocation prop changes (e.g. switching days)
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col" style={containerStyle}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`w-full md:w-[340px] shrink-0 overflow-y-auto border-r border-slate-100 bg-white ${activeView === 'map' ? 'hidden md:flex md:flex-col' : 'flex flex-col'}`}>
          ...
          {!loading && results.length === 0 && (
            <div className="flex flex-col items-center justify-center flex-1 gap-3 text-center p-8">
              ...
              <div>
                <p className="font-semibold text-slate-700">Discover places nearby</p>
                <p className="text-sm text-slate-400 mt-1">
                  Search for cafes, museums, restaurants and more — then add them to your itinerary.
                </p>
              </div>
            </div>
          )}
